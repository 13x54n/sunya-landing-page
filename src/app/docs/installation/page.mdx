import { siteDetails } from "@/data/siteDetails"

export const metadata = {
  title: "Installation",
  description: "Install Sunya for EVM smart contract security scanning. One-command curl install for Linux and macOS. PowerShell or CMD install for Windows. No npm required.",
  openGraph: {
    title: "Installation | Sunya",
    description: "Install Sunya for static analysis of EVM smart contracts. Works on Linux, macOS, and Windows.",
    url: `${siteDetails.siteUrl}/docs/installation`,
  },
  alternates: { canonical: `${siteDetails.siteUrl}/docs/installation` },
}

# Installation

Sunya works on **Linux**, **macOS**, and **Windows**.

## Linux & macOS

```bash
curl -sSL https://raw.githubusercontent.com/13x54n/sunya-v1/main/package/bin/install | sh
```

Add `~/.local/bin` to PATH if needed:

```bash
# macOS (zsh)
echo 'export PATH="$HOME/.local/bin:$PATH"' >> ~/.zshrc

# Linux (bash)
echo 'export PATH="$HOME/.local/bin:$PATH"' >> ~/.bashrc
```

## Windows

**PowerShell:**
```powershell
irm https://raw.githubusercontent.com/13x54n/sunya-v1/main/package/bin/install.ps1 | iex
```

**CMD:**
```cmd
curl -sL -o %TEMP%\sunya-install.cmd https://raw.githubusercontent.com/13x54n/sunya-v1/main/package/bin/install.cmd && %TEMP%\sunya-install.cmd
```

Restart your terminal after installation so PATH updates take effect.

## Commands

- `sunya scan [dir]` — Analyze contracts (default: ./contracts or ./src)
- `sunya install` — Install or update Slither
- `sunya uninstall [-y]` — Remove sunya (use `-y` to skip confirmation)

## Uninstall

Works on all platforms:

```bash
sunya uninstall      # Prompts for confirmation
sunya uninstall -y   # Remove without prompting
```

## Requirements

- **Linux, macOS:** Bash, Node.js (recommended) or Python
- **Windows:** Node.js
- **All platforms:** Python 3 (for Slither)
